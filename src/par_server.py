...
CLIENT_HOST = '160.39.216.107'#200.99' # IP address of the HTC One M8
...
def genHighlightAndSend(index):
    genHighlights(index)
    try:
        sendData(getHighlightFilename(index))
    except:
        print('Could not send highlight for video %d' % index)

def clientWrapper1(index):
    while (True):
        # get(block=True) blocks until an item is available
        index = q1.get(block=True)
        genHighlightAndSend(index)

def clientWrapper2(index):
    while (True):
        index = q2.get(block=True)
        genHighlightAndSend(index)
...
if __name__ == '__main__':
    ...
    p1 = Process(target=clientWrapper1, args=(q1,))
    p1.start()
    p2 = Process(target=clientWrapper2, args=(q2,))
    p2.start()
