...
BACK_PORT = 8000               # port number for the back camera video
FRONT_PORT = 8888              # port number for the front camera video
CONTROL_PORT = 8080            # port number for any control information
HIGHLIGHT_PORT = 8800          # port number for the highlight video
...
def receiveData(s, name):
    ...
    while (True):
        ...
        if (addr[0] != CLIENT_HOST):
            ...
            print('[receiveData] ' + name + 'Connected with %s: %s' % (addr[0], str(addr[1])))
            sys.stdout.flush()
        while (True):
            data = conn.recv(SERVER_BUFSIZE)
            if (data):
                print('[receiveData] ' + name + 'Received %d bytes' % len(data))
                sys.stdout.flush()
                buffer += bytearray(data)
            else:
                print('[receiveData] ' + name + 'Finished receiving data')
                sys.stdout.flush()
                return buffer
    
def receiveBackVideoData():
    global back_socket
    return receiveData(back_socket, "back camera")
    
def receiveFrontVideoData():
    global front_socket
    return receiveData(front_socket, "front camera")

def receiveControlData():
    global control_socket
    return receiveData(control_socket, "control")
...
def clientWrapper(index):
    while (True):
        ...
        index = q.get(block=True)
        genHighlights(index)
        sendData(getHighlightFilename(index))
